...
export const findPurchasesByUserId = async (userId, page = 1) => {
   return await pool.query(
      `SELECT 
         JSON_ARRAYAGG(JSON_OBJECT(
      ...
      FROM purchases P
         INNER JOIN (
            SELECT
               pp.purchase_id,
               JSON_ARRAYAGG(JSON_OBJECT(
                  'product_id', pp.product_id, 'img_url', Pr.img_url, 'quantity', pp.quantity ,'name', Pr.name, 'description', Pr.description, 'sale_price', Pr.sale_price
               )) products
            FROM purchase_products pp
               INNER JOIN products Pr ON Pr.product_id=pp.product_id
            GROUP BY pp.purchase_id
         ) PP ON PP.purchase_id=P.purchase_id
      WHERE P.user_id= ? LIMIT ${(page - 1) * 20}, 20`,
      [userId]
   );
};

export const countPurchasesByUserId = async (userId) => { 
   return await pool.query(
      'SELECT COUNT(purchase_id) as purchases_count FROM purchases WHERE user_id = ?',
      [userId]
   );
};
